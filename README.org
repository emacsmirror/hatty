...
The bounds are markers, ensuring stability of the positions across
...
symbols used to invoke ~hatty-locate-token~.  The special identifier
...
:CUSTOM_ID: hat-sizes
:END:
If you want to change hat sizes, set ~hatty-scale-factor~.  The size
of the rendered hats are multiplied by this factor.
